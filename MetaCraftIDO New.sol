...
contract MetaCraftIDO is ReentrancyGuard {
   ...
   function deleteDelUserTx(uint transactionId)
    ...
    {
                chainId = block.chainid;
                bytes32 challenge = getChallenge(timestamp, nonce,userLevel, msg.sender,chainId);
    ...
    function getChallenge(uint256 timestamp,
        ...
        bytes32 challenge = keccak256(abi.encodePacked( userAddress, timestamp, nonce, userLevel, currentchainId));
